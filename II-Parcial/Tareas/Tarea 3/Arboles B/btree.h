...
void btree::Borrar(int valor)
{
    ...
    {
        i = 0;
        while (i < nodo->clavesUsadas && (nodo->clave[i].valor < valor))
            i++;
        if (nodo->clave[i].valor == valor && i < nodo->clavesUsadas)
            encontrado = true;
        else
            nodo = nodo->puntero[i];
    }
    if (encontrado)
        BorrarClave(nodo, valor);
}
void btree::BorrarClave(pbnodo nodo, int valor)
{
    ...
    pos = 0;
    while (nodo->clave[pos].valor < valor)
        pos++;
    if (nodo->puntero[0])
    {
